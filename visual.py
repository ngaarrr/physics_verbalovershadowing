...
def visualize(
    ...
    # --- PREVIEW DELAY: Show initial frame before drop ---
    preview_frame = sim_data["ball_position"][0]
    screen.fill(THECOLORS["white"])
    colors = [THECOLORS["blue"], THECOLORS["red"], THECOLORS["green"]]
    draw_obstacles(rotated, screen, colors, c)
    draw_ground(c, screen)
    draw_bins(c, screen)
    if not (eye_pos is None):
        draw_eye(c, screen, eye_pos)
    draw_ball(c, screen, preview_frame)
    draw_walls(c, screen)
    pygame.event.get()
    pygame.display.flip()
    ...
    if make_video:
        ...
        sp.call(
            "ffmpeg -y -framerate 60 -i {ims}  -c:v libx264 -profile:v high -crf 10 -pix_fmt yuv420p {videoname}.mp4".format(
                ims=img_dir + "/'%05d.png'", videoname="data/videos/" + video_name
            ),
            shell=True,
        )
    ...
    if auto_close:
        # Auto-close after a short delay when ball reaches ground
        pygame.time.wait(500)  # Wait 500ms after ball settles
        pygame.quit()
    else:
        # Original behavior - wait for user input
        running = True
        while running:
...
def rotate_shapes(c):
    ...
    for shape in c["obstacles"]:
        ...
        if "shape" not in c["obstacles"][shape]:
            poly = utils.generate_ngon(
                c["obstacles"][shape]["n_sides"], c["obstacles"][shape]["size"]
            )
...
def draw_box_unity(c, screen, adjust=False):
    ...
    # Calculate hole positions dynamically for any number of holes
    num_holes = len(c["old_hole_positions"])
    hole_left_positions = [pos - c["hole_width"] / 2 for pos in c["old_hole_positions"]]
    hole_right_positions = [
        pos + c["hole_width"] / 2 for pos in c["old_hole_positions"]
    ]
    ...
    # Draw top wall segments between holes
    # First segment: from left wall to first hole
    unity_hole1_left, _ = convertCoordinate(hole_left_positions[0], pymunk_y_top)
    pygame.draw.line(
        screen,
        THECOLORS["black"],
        (unity_x_left, unity_y_top),
    ...
    # Middle segments: between holes
    for i in range(num_holes - 1):
        unity_hole_right, _ = convertCoordinate(hole_right_positions[i], pymunk_y_top)
        unity_hole_left, _ = convertCoordinate(hole_left_positions[i + 1], pymunk_y_top)
    pygame.draw.line(
        screen,
        THECOLORS["black"],
        (unity_hole_right, unity_y_top),
        (unity_hole_left, unity_y_top),
    )

    # Last segment: from last hole to right wall
    unity_hole_last_right, _ = convertCoordinate(
        hole_right_positions[num_holes - 1], pymunk_y_top
    )
    pygame.draw.line(
        screen,
        THECOLORS["black"],
        (unity_hole_last_right, unity_y_top),
        (unity_x_right, unity_y_top),
    )


def draw_obstacles_unity(c, screen, colors):
    for name, ob_dict in c["obstacles"].items():
        if name == "abstract" and "arms" in ob_dict:
            arms = ob_dict["arms"]
            for arm in arms:
                pygame.draw.line(
                    screen,
                    "black",
                    (int(arm[0][0]), int(utils.flipy(c, arm[0][1]))),
                    (int(arm[1][0]), int(utils.flipy(c, arm[1][1]))),
                    14,
                )
                pygame.draw.line(
                    screen,
                    "black",
                    (int(arm[1][0]), int(utils.flipy(c, arm[1][1]))),
                    (int(arm[2][0]), int(utils.flipy(c, arm[2][1]))),
                    14,
                )
        else:
...
def draw_walls(c, screen):
    ...
    for i in range(num_holes - 1):
        pygame.draw.line(
            screen,
            THECOLORS["black"],
            (c["hole_positions"][i] + c["hole_width"] / 2, top_wall_y),
            (c["hole_positions"][i + 1] - c["hole_width"] / 2, top_wall_y),
        )
    ...
    pygame.draw.line(
        screen,
        THECOLORS["black"],
        (c["hole_positions"][num_holes - 1] + c["hole_width"] / 2, top_wall_y),
        (c["med"] + c["width"] / 2, top_wall_y),
    )
...
def draw_bins(c, screen):
    # Bin dimensions
    bin_width = c["width"] // 2
    bin_height = 30  # Height of the bins (reduced from 60 to 30)
    ground_y = c["med"] - c["height"] / 2  # y position of the ground

    # Left bin
    left_bin_rect = pygame.Rect(
        c["med"] - c["width"] // 2,  # left x
        utils.flipy(c, ground_y) - bin_height,  # top y (pygame y is flipped)
        bin_width,  # width
        bin_height,  # height
    )
    pygame.draw.rect(screen, THECOLORS["green"], left_bin_rect, 0)  # filled

    # Right bin
    right_bin_rect = pygame.Rect(
        c["med"],  # left x (center)
        utils.flipy(c, ground_y) - bin_height,  # top y
        bin_width,  # width
        bin_height,  # height
    )
    pygame.draw.rect(screen, THECOLORS["red"], right_bin_rect, 0)  # filled

    # Divider (dark black, slightly higher than bins)
    divider_top = (
        utils.flipy(c, ground_y) - bin_height - 10
    )  # 10 pixels higher than bins (reduced from 20)
    divider_bottom = utils.flipy(c, ground_y)
    pygame.draw.line(
        screen,
        THECOLORS["black"],
        (c["med"], divider_top),
        (c["med"], divider_bottom),
        6,
    )


def draw_obstacles(rotated, screen, colors, c=None):
    for idx, shape in enumerate(rotated):
        # Always draw filled black polygons for all obstacles (including abstract)
...
def draw_eye(c, screen, eye_pos):
    ...
    pygame.draw.circle(
        screen, THECOLORS["black"], (eye_pos[0], utils.flipy(c, eye_pos[1])), 3, width=0
    )
...
def generate_trial_shapes(trial, generate_shapes=True):
    ...
    for ob, ob_dict in shape_trial["obstacles"].items():
        ...
        if generate_shapes:
            ob_shape = np.array(
                utils.generate_ngon(ob_dict["n_sides"], ob_dict["size"])
            )
...
def graph_conditional_dist(
    ax,
    hole,
    density_samples,
    multiplier=8000,
    offset=465,
    unity_background=False,
    kde_method="FFT",
):
    ...
    if kde_method == "FFT":
        ...
        kde = KernelDensity(kernel="gaussian", bandwidth=20).fit(
...
def visualize_frame(
    ...
    if save:
        plt.savefig("visuals_agent/frames/{}.png".format(frame_name), dpi=200)
...
def visualize_trial(
    df_trial, trial_num, trial, viz_type="pdf", frame_rate=3, kde_method="FFT"
):
    ...
    if viz_type == "pdf":
        ...
        imgs[0].save(
            "visuals_agent/trials/trial" + str(trial_num).zfill(3) + ".pdf",
            save_all=True,
            quality=100,
        ...
        subprocess.run(
            "ffmpeg -framerate {} -i visuals_agent/frames/frame%03d.png -c:v libx264 -profile:v high -crf 10 -pix_fmt yuv420p visuals_agent/trial_videos/trial{}.mp4".format(
                frame_rate, str(trial_num).zfill(3)
            ).split(
                " "
            )
        )
